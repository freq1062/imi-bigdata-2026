"""Utilities to prepare resources for first-time training runs."""
...
from lib.resource_paths import PROJECT_ROOT, DATA_DIR, OUTPUTS_DIR
...
def ensure_webapp_dirs() -> None:
    ...
    (OUTPUTS_DIR / "rf_model_sage").mkdir(parents=True, exist_ok=True)


def ensure_scotia_data(*, force_download: bool = False, quiet: bool = False) -> list[str]:
    """Ensure required Scotia source files exist in resources/data.
    ...
    if not missing and not force_download:
        if not quiet:
            print(f"All data files already present in {DATA_DIR}")
        return []
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Please place the .csv.gz files in resources/data/."
        ) from exc
    ...
    still_missing = [f for f in REQUIRED_DATA_FILES if not (DATA_DIR / f).exists()]
...
def package_webapp_model_artifacts(
    *,
    ...
    """Copy trained model artifacts into resources/outputs.
